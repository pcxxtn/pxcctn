 ...
 * 日期 2018-04-23 17:04
 */
@Service("breakPkgService")
public class BreakPkgService implements IPkg {
...
//            String data = message;
    ...
    private void unPack(List<FldBean> list, String dataTmp, String chnlno, int row, TransVariableSet varSet) {
        ...
        for (FldBean fldBean : list) {
            if (fldBean.getProcfunc().equals("000000|0")) {
                ...
                log.info("截取的报文字段:" + value + " 放入变量池: {key:" + fldBean.getCommstructfldname() + " value:" + value + " row:" + row + "}");
    ...
    public String fillBreak(FldBean fldBean, int index, TransVariableSet varSet) {
        ...
        log.info("打包字典名称：" + fldBean.getCommstructfldname() + "  变量池取变量： {key: " + fldBean.getCommstructfldname() + " value:" + obj.toString() + " row:" + index + "}");
        return value;
    }
